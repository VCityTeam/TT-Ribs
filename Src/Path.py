...
logging.basicConfig(filename="Path.log", encoding="utf-8", level=logging.INFO)


class Cave:
    IDENTIFICATION_THRESHOLD = 5.1  # Totally empirical and ad-hoc to this Cave
    blender_pathfile = "../Blender/Cave_V6-2.blend"
    ...
    def __apply_modifiers(self):
        ...
        with bpy.context.temp_override(
            selected_objects=[self.path], object=self.path, active_object=self.path
        ):

            # stalactites = self.path.modifiers["SimpleDeform"]
            # stalactites.factor = self.slactatite_strech_factor
            bpy.ops.object.modifier_apply(modifier="SimpleDeform")

            # subdiv = self.path.modifiers["Subdivision"]
    ...
    def __fill_holes(self):
        ...
        cave_bmesh = bpyhelpers.UI_demote_UI_object_with_mesh_to_bmesh(self.path)
        ...
        cave_bmesh.to_mesh(self.path.data)
    ...
    def __replicate_to_build_grid(self):
            ...
            + str(
                len(
                    bpyhelpers.bmesh_get_boundaries(
                        bpyhelpers.UI_demote_UI_object_with_mesh_to_bmesh(self.path)
                    )
                )
            ),
        )
        ...
        with bpy.context.temp_override(
            ...
            if self.grid_size_x > 1:
                copier_x = self.path.modifiers["Array_Y"]
            ...
            if self.grid_size_y > 1:
                copier_y = self.path.modifiers["Array_X"]
    ...
    def __assert_resulting_topology(self):
        ...
        """
        resulting_bmesh = bpyhelpers.UI_demote_UI_object_with_mesh_to_bmesh(self.path)
